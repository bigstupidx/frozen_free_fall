...
public class ColorBombTile : TriggerTile
{
	...
	{	
		if (useDestroyEffect && useEffect) {
			...
			if (freezeEffect)
			{
				freezeEffect.animation.Play("effect_winterchill_destruction_freeze");				
			}
			
			animation.Play("effect_winterchill_destruction");
	...
	{
		yield return new WaitForSeconds(animation["effect_winterchill_idle"].length);
		
		if (!IsDestroying) {
			animation.Play("effect_winterchill_idle");
		}
	}
