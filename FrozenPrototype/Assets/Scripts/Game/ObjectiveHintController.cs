...
public class ObjectiveHintController : MonoBehaviour {
	...
	public void Awake()
	{
		...
		Debug.Log("[MarkObjectiveController_END] Registered to OnStableBoardCheckingStatusChanged...");
	}
	
	public void Start()
	{
		Debug.Log("[MarkObjectiveController] START START Registered to OnStableBoardCheckingStatusChanged...");
	}
...
//		Debug.Log("[ObjectiveHintController] OnStableBoardCheckingStatusChangedEvent...");
		...
		if ( !isCheckingBoardStable )  {
			...
			effectMarkObjective.animation.Stop();
		}
	}
	...
	IEnumerator ShowHintAfterDelay(float delay) 
	{
		...
		if (!Match3BoardGameLogic.Instance.IsGameOver)
		{
			...
			effectMarkObjective.animation.Play();
		}
	}
