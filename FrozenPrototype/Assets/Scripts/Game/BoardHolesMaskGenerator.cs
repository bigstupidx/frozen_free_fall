...
public class BoardHolesMaskGenerator
{
	...
	//-----------------------------------------------------
	public Vector2 	uvTextureGridStart = new Vector2(0.1276f, 0.0911f);
	public Vector2 	uvTextureSquaresEnd	  = new Vector2(0.9088f, 0.8717f);
...
//					texture.SetPixel(x, y, ColorValue(alphaNew));
	...
	void ReadGridCellsInfoFromBoard()
	{
		for(int row = 0; row < boardData.NumRows; row++) {
			for(int col = 0; col < boardData.NumColumns; col++) {
				gridEmptyCells[row, col] = boardData[row, col].IsEmpty;
			}
		}
	}
	...
	//-----------------------------------------------------
	void BakeMaskSoft(Texture2D mask)
	{
		ComputeGridCellSize();
