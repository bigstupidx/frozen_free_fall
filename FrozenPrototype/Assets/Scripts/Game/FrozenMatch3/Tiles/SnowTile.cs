...
public class SnowTile : NormalTile {
	...
	protected int NumLayers {
		...
		set
		{
			...
			if(numberOfSnowLayers < oldNumberOfSnowLayers && numberOfSnowLayers >= 0)
			{
				...
				animation.PlayQueued(animationClipNames[0]);
			}
		}
	}
	...
	{		
		...
		{
			snowDestroyEffect.cachedModelRenderer.renderer.material = materialArray[numberOfSnowLayers-1];
		...
		yield return new WaitForSeconds(animation[animationClipNames[0]].length);
		...
		{
			tileModelRenderer.material = materialArray[numberOfSnowLayers-1];
			animation.Play(animationClipNames[1]);
		}
	}
