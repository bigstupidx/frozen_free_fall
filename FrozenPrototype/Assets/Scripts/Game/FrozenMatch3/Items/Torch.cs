...
public class Torch : BasicItem {
	...
	//Is true when actually using the torch and false after it finishes its anim.
	...
	protected IEnumerator DoItemCoroutine()
	{
		...
		torchEffectAnimation = torchEffectInstance.animation;
		
		torchEffectAnimation.Play();
		
		yield return new WaitForSeconds(torchEffectInstance.animation.clip.length + postFadeInWaitTime);
	...
	protected IEnumerator EndOfPathMovement()
	{
		...
		yield return new WaitForSeconds(torchEffectInstance.animation.clip.length);
