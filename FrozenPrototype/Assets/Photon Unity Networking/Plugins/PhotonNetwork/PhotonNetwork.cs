...
public static class PhotonNetwork
{
    ...
    public static void ConnectUsingSettings(string gameVersion)
    {
        ...
        {
            UnityEngine.Debug.LogError("Can't connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetwork.serverSettingsAssetFile);
            return;
        }
    ...
    public static void Connect(string serverAddress, int port, string appID, string gameVersion)
    {
        ...
        {
            UnityEngine.Debug.LogWarning("Connect() only works when disconnected. Current state: " + networkingPeer.PeerState);
            return;
        }
    ...
    public static void JoinRoom(RoomInfo listedRoom)
    {
        if (listedRoom == null)
        {
            UnityEngine.Debug.LogError("JoinRoom aborted: you passed a NULL room");
            return;
        }
    ...
    public static void UnAllocateViewID(int viewID)
    {
        ...
        {
            UnityEngine.Debug.LogWarning(string.Format("Unallocated manually used viewID: {0} but found it used still in a PhotonView: {1}", viewID, networkingPeer.photonViewList[viewID]));
        }
    }
    ...
    public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, int group, object[] data)
    {
        ...
        if (viewIDs == null)
        {
            UnityEngine.Debug.LogError("PhotonNetwork error [InstantiateSceneObject]: Could not Instantiate the prefab [" + prefabName + "] as no ViewIDs are free to use. Max is: " + MAX_VIEW_IDS);
            return null;
        }
